...
import {
  useIncAllComponentIndices,
  useIncComponentIndex,
  useInitComponentIdx,
} from "../../hooks/componentIndexHooks";
...
const dialogue: Dialogue[] = [
  {
    text: "Now that the party is ready, you are free to do whatever until the guests arrive. Make sure you stay in this area though, just in case",
    image: pumpkinHead,
    audio: pumpkinHeadMusic,
  },
  {
    text: "I'm sure that timer at the top means absolutely nothing and there is nothing to worry about!",
    image: "",
    audio: song,
  },
  { text: "...", image: "" },
  {
    text: "Maybe it'd be a good idea to ask him some questions now. Be warned, if you ask aimless questions, your time will run out faster...",
    image: "",
  },
...
const TimerGame = ({}: {}) => {
  ...
  const [playerComponentIndex, setPlayerComponentIndex] =
    useState(componentIndex);
  ...
  const handleNext = async () => {
    ...
    if (
      playerComponentIndex === componentIndex &&
      componentIndex >= dialogue.length - 1
    ) {
      incAppIndex();
      return;
    }
  ...
  const ShowOptions = () => {
    ...
    const handleWrongAnswer = async () => {
      ...
      setWrongAns(true);
      const start = await (
        await get(ref(db, dbCode + "/" + "/middleTimer"))
      ).val();
      if (start) {
      ...
      <div>
        ...
        {showInput && !wrongAns ? (
          <div>
            ...
            <Button color="primary" variant="contained" onClick={inputCurse}>
              Submit
            </Button>
          </div>
        ) : (
          !wrongAns && (
            ...
            >
              ...
              <Button color="primary" variant="contained" onClick={handleCurse}>
                The curse
              </Button>
              <Button
                color="primary"
  ...
  return (
    <div>
      ...
      {showOptions ? (
        ...
        <Dialogue text={el.text} image={el.image} />
      )}
      {showOptions ? (
        <div></div>
      ) : (
