...
const SoupGame = () => {
    ...
    const handleSubmit = () => {
        ...
        if (ans === "abc") {
            ...
            if (ans === exampleCode) {
                setWarn("You really thought the answer would be the example I gave?! Take two penalties :/")
            } else {
    ...
    return <div>
        <Dialogue header={"TIME FOR SOUP"}
            text={[""]} image={cauldronImg}/>
            <p>Every good party needs Witch's Brew! Look at the recipe on the table and try to match it to the potions on the table (do not write on the recipe)</p>
            <p>When you think you've solved the riddle, enter the number/letter of the potion in order in the input field. For example, {exampleCode}</p>
